...
\begin{enumerate}
    ...
    \item \textbf{Editar el \glsnocase{Footprint}.}\\
...
De forma que quedará como podemos ver en la figura \ref{fig:ESP32S3HOLE}.
...
La idea siempre ha sido ajustar las dimensiones de la carcasa y \gls{PCB} para que no se pudiera ver la \gls{PCB} una vez que el teclado tuviese las \glsnocase{Keycaps} montadas. Esto gracias a que la \glsnocase{Plate} no dejaría mucho hueco para ver la \gls{PCB}, que se vería peor estéticamente. Por eso, uno de los objetivos es intentar que la carcasa esté lo más cerca de las \glsnocase{Keycaps}.
\newpage
...
Como podemos ver en el código \ref{code:CodigoDebug} esta es la solución por la que se ha optado y también se puede encontrar en el GitHub del proyecto \cite{ModernWoodGitHub}. 
